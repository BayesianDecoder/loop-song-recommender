...
# set paths
# output paths
track_ids_save_path = "data/track_ids.npy" # numpy array
filtered_data_save_path = "data/collab_filtered_data.csv"
interaction_matrix_save_path = "data/interaction_matrix.npz" # multiple numpy array 
# input paths
...
def collaborative_recommendation(song_name,artist_name,track_ids,songs_data,interaction_matrix,k=5):
    ...
    top_k_songs = (
                    songs_data
                    .loc[songs_data["track_id"].isin(recommendation_track_ids)] # filters the song_data based on recommendation_track_ids
                    .merge(scores_df,on="track_id") # joins the filtered songs with corresponding scores
                    .sort_values(by="score",ascending=False) #now sorts the the values by score 
                    .drop(columns=["track_id","score"])# removes these columns
